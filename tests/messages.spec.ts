...
test.describe('Customer and Admin Operations', () => {
    ...
    test('Verify message flow between admin and customer', async ({ browser }) => {
        ...
        await customerPage.goto('https://practicesoftwaretesting.com/#/account/messages');
        await customerPage.locator(`(//td[text()='${lowerCasemsgSubject}']/ancestor::tr)[1]//a`).click();
        await expect.soft(customerPage.locator(`//h2[text()='Replies']`)).toBeVisible();
        await expect.soft(customerPage.locator(`(//p[text()='Hi, This is a test reply for customer from admin.'])[1]`)).toBeVisible();
    });
});
